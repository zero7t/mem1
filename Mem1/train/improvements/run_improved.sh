#!/bin/bash
# Improved training script with DAPO-lite + Dr.GRPO + LLDS-MA
# Changes from run_fix_test.sh:
#   - batch_size=96 (was 60), fallback to 60 if OOM
#   - save_freq=100
#   - experiment_name includes "IMPROVED"
#   - New config: actor.llds_lambda=0.1, actor.clip_higher=0.28
...
exec python -m verl.trainer.main_ppo \
  ...
  trainer.experiment_name=IMPROVED-DAPO-DrGRPO-LLDS \
  trainer.total_epochs=1 \
  trainer.total_training_steps=1413 \
  trainer.default_local_dir=verl_checkpoints/IMPROVED-DAPO-DrGRPO-LLDS \
  max_turns=6 \
