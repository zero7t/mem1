#!/bin/bash
# V4.3 Resume from step 300: fix turn-weight advantage (V3 multiplicative style)
# Changes from V4.2:
#   - Fix advantage distribution: V3 multiplicative (no length bias) instead of budget/num_tokens
#   - Keep all other V4.2 fixes: efficiency weight 0.1, fast-wrong penalty, grad_clip=1.0, LLDS action-level
...
python3 -m grpo_v3.main_ppo_v3 \
  ...
  trainer.experiment_name=V4.3-grpo-n4-bs96-resume300 \
  trainer.total_epochs=1 \
  trainer.total_training_steps=800 \
  trainer.default_local_dir=verl_checkpoints/V4.3-grpo-n4-bs96-resume300 \
  max_turns=6 \
  do_search=true \
