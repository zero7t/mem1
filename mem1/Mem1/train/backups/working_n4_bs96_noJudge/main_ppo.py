...
class RewardManager():
    ...
    def _process_item(self, i, data_item, already_print_data_sources):
        ...
        try:
            ...
            pass  # Silently skip process reward on errors

        # === LLM Outcome Judge (when EM=0) === [DISABLED FOR TIMING TEST]
        # if score < 0.5:
        #     try:
        ...
        #                 response = self._llm_judge._call_api_sync(prompt)
        #                 if response and "correct" in response.lower() and "incorrect" not in response.lower():
        #                     score = 0.8 + process_score if 'process_score' in dir() else 0.8
        #     except Exception as e:
        #         pass
