...
import re
from rollout.llm_agent.generation_webagent import LLMGenerationManager, GenerationConfig
    ...
    def _validate(self):
        ...
        if not self.config.do_search:
            for test_data in self.val_dataloader:
                ...
                test_gen_batch = test_batch.pop(batch_keys=['input_ids', 'attention_mask', 'position_ids', 'environment_ids'])
            ...
            for batch_dict in tqdm.tqdm(self.val_dataloader):
                ...
                test_gen_batch.batch['environment_ids'] = test_gen_batch.batch['environment_ids'].unsqueeze(1)
                test_gen_batch.meta_info = {
                    'eos_token_id': self.tokenizer.eos_token_id,
                    'pad_token_id': self.tokenizer.pad_token_id,
                    'recompute_log_prob': False,
        ...
        peak_seq_token_len = final_gen_batch_output.meta_info['peak_seq_token_len']
        average_token_len_per_turn = final_gen_batch_output.meta_info['average_token_len_per_turn']

        metric_dict['val/peak_seq_token_len'] = np.mean(peak_seq_token_len)
        metric_dict['val/average_token_len_per_turn'] = np.mean(average_token_len_per_turn)
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                gen_batch = batch.pop(batch_keys=['input_ids', 'attention_mask', 'position_ids', 'environment_ids'])
                ...
                # with _timer('step', timing_raw):
                        ...
                        gen_batch.batch['environment_ids'] = gen_batch.batch['environment_ids'].unsqueeze(1)
                        ...
                        with _timer('gen', timing_raw):
                            ...
                            peak_seq_token_len = final_gen_batch_output.meta_info['peak_seq_token_len']
                            average_token_len_per_turn = final_gen_batch_output.meta_info['average_token_len_per_turn']
                            metrics['peak_seq_token_len'] = np.mean(peak_seq_token_len)
                            metrics['average_token_len_per_turn'] = np.mean(average_token_len_per_turn)
