#!/bin/bash
# V4.2 Resume from step 300: efficiency reward fix (0.3->0.1, add fast-wrong penalty)
# Changes from V4.1:
#   - efficiency_reward weight 0.3 -> 0.1
#   - Add penalty for fast-wrong: -0.1 * (max_turns - num_turns) / max_turns
#   - Resume from V3 step 300 checkpoint
...
python3 -m grpo_v3.main_ppo_v3 \
  ...
  trainer.experiment_name=V4.2-grpo-n4-bs96-resume300 \
  trainer.total_epochs=1 \
  trainer.total_training_steps=800 \
  trainer.default_local_dir=verl_checkpoints/V4.2-grpo-n4-bs96-resume300 \
  max_turns=6 \
  do_search=true \
  ...
  2>&1 | tee V4.2-grpo-n4-bs96-resume320.log
